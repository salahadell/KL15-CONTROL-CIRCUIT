...
uint8 isGreen_ON(){
	uint8 x;
	DIO_ReadPin(GREEN_LED_PORT,GREEN_LED_PIN,&x);
	return x;
}
uint8 isBlue_ON(){
	uint8 x;
	DIO_ReadPin(BLUE_LED_PORT,BLUE_LED_PIN,&x);
	return x;
}
uint8 isRed_ON(){
	uint8 x;
	DIO_ReadPin(RED_LED_PORT,RED_LED_PIN,&x);
	return x;
}
uint8 isYellow_ON(){
	uint8 x;
	DIO_ReadPin(YELLOW_LED_PORT,YELLOW_LED_PIN,&x);
	return x;
}
